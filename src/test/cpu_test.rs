...
fn test_new() {
    ...
    let c: Cpu = Cpu::new(None, None, false).unwrap();
    assert_eq!(c.sp, 0x0);
    ...
    let c: Cpu = Cpu::new(
        Some(ROM::new_prog(&[0xde, 0xad, 0xbe, 0xef]).unwrap()),
        None, false
    ).unwrap();
    assert_eq!(c.sp, 0x0);
...
fn test_cpu_reset() {
    // reset on no program
    let mut c = Cpu::new(None, None, false).unwrap();
    c.ram[0x200] = 0x42;
    c.pc = 0xdead;
    c.i = 0xbeef;
    c.reset().unwrap();
...
fn test_0x2nnn() {
    ...
    assert_eq!(c.s[c.sp], 0x202); // return address after call
